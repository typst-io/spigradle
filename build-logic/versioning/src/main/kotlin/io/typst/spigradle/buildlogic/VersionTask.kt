...
abstract class VersionTask : DefaultTask() {
    companion object {
        fun findVersionFile(dir: File): File? {
            val file = dir.resolve("version.txt")
            if (file.isFile) {
                return file
            }
            val parentFile = dir.parentFile ?: return null
            return findVersionFile(parentFile)
        }

        fun readVersion(project: Project): String {
            val file = findVersionFile(project.rootDir)
            return readVersion(file!!)
        }

        fun readVersion(file: File): String = file.readText().trimEnd('\r', '\n')
    }
